...
---
에이전트는 어떻게 기억력을 가질까요? 우리는 에이전트의 짧은 기억력에 쉬이 불만을 토로하지만, 에이전트가 정확히 무슨 문제를 겪고 있는지는 잘 모릅니다. 

기억에는 종류가 있습니다. 어제 나눈 대화를 기억하는 것과, 사용자의 취향을 기억하는 것, 이 작업은 이 순서로 해야 한다를 기억하는 것은 다른 문제입니다. 각각 다른 구조가 필요하고, 다른 도구가 필요합니다.

> 이 글은 [[AAIF]]가 정리한 에이전트 메모리 7가지 유형을 기준으로, LLM Wiki가 어디에 해당하는지, mem0 같은 도구가 무엇을 해결하는지를 정리합니다.

메모리의 종류에 따라 아키텍처가 달라집니다.
