...
트랜스포머와 어텐션 메커니즘은 뛰어난 성능으로 레전드가 되었습니다. 그런 어텐션 메커니즘에도 약점이 많이 있습니다. 특히 길이가 긴 문맥을 다룰 때 시퀀스 길이 $n$에 대한 $O(n^2)$ 시간복잡도는 치명적인 약점입니다. 시간이 흐르고 LLM이 백만 토큰 규모의 입력을 처리하게 되면서 이 단점이 서서히 드러났습니다. 강화학습(RL) 기반의 추론 스케일링이 중요해지면서 이 문제는 더욱 심각해졌죠.

![[블로그/markdown-blog/grid_Papers/_assets/1-KimiLinear.png|431x416]]
