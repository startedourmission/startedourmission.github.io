...
2026년 4월 19일, [[Vercel]]이 공식 Knowledge Base에 조용히 공지를 올렸습니다. 내부 시스템 일부에 무단 접근이 있었고, 일부 고객의 환경변수(environment variables)가 노출됐다는 내용이었습니다. 공지만 보면 담담하지만, 내부 사정은 그렇지 않았습니다. 침해 경로가 "작은 서드파티 AI 도구"였다는 점 때문입니다. 이 사고가 왜 중요한지, 그리고 Vercel 쓰는 우리가 지금 무엇을 해야 하는지 정리해봤습니다.
...
이 패턴에는 이름이 있습니다. **OAuth 공급망 공격**입니다.

공격자가 서비스 자체를 뚫을 필요가 없습니다. 그 서비스를 쓰는 조직이 신뢰하는 앱 중 가장 약한 곳을 찾아 들어옵니다. 전통적인 소프트웨어 공급망 공격(코드 주입, 악성 패키지)과 달리, 이 방식은 코드를 전혀 건드리지 않습니다. 허가된 OAuth 토큰만 씁니다. 그래서 탐지가 어렵습니다. 공격 트래픽이 정상 API 호출처럼 보입니다.

공격자는 ShinyHunters로 알려진 그룹입니다. 이들은 2024~2025년 Snowflake, Ticketmaster, AT&T 등 60개 이상의 기업 침해에 연루됐습니다. 방식이 반복됩니다. 고객 계정 자격증명 또는 통합 앱을 통해 SaaS 플랫폼에 들어가고, 데이터를 대량으로 빼낸 뒤 사이버범죄 포럼에서 판매합니다. Vercel도 그 패턴의 연장선입니다.

AI 도구의 급증이 이 문제를 구조적으로 악화시키고 있습니다. 2025~2026년 사이 팀이 쓰는 SaaS AI 도구의 수가 급격히 늘었습니다. 각각의 도구는 온보딩 시 OAuth 동의를 요청합니다. "이 앱이 당신의 Google Drive, Calendar, Gmail에 접근합니다"에 무심하게 Allow를 누릅니다. 그 순간 우리는 그 앱 개발사의 보안 수준을 우리 조직의 보안 수준에 묶습니다.

문제는 규모입니다. 팀원 10명이 각자 5개의 AI 도구를 쓴다면, 조직은 50개의 외부 앱에 Google Workspace 권한을 준 상태입니다. 그 중 가장 보안이 약한 한 곳만 뚫리면, 공격자는 연결된 모든 것으로 들어올 수 있습니다. AI 도구 → Google Workspace → 계정 → Vercel → 고객 환경변수. Vercel이 아무리 잘 지켜도 막을 수 없는 경로입니다.
...
Vercel은 "services remain operational"이라고 했습니다. 기술적으로는 맞습니다. 다만 이 사고가 남긴 질문은 기술보다 구조에 가깝습니다.

우리 조직이 쓰는 수십 개의 SaaS·AI 도구 중, 침해됐을 때 이런 연쇄 반응을 일으킬 만한 권한을 가진 앱이 몇 개나 되는지 답할 수 있을까요. 답이 "모르겠다"에 가깝다면, 이번 공지는 남의 이야기가 아닙니다.

Vercel의 대응 자체는 빠르고 투명한 편이었습니다. OAuth 앱 ID까지 IOC로 공개했고, 법 집행기관에 통보했으며, 조사를 계속 진행 중이라고 밝혔습니다. 그러나 개별 고객 입장에서는 Vercel의 투명성과 무관하게 해야 할 일이 남습니다.

더 큰 문제는 이 공격 방식 자체가 앞으로 더 자주 쓰일 것이라는 점입니다. AI 도구 채택 속도는 보안 감사 속도보다 빠릅니다. 대부분의 팀에서 새 AI 도구 도입과 OAuth 승인 사이에 보안 검토가 없습니다. ShinyHunters 같은 그룹은 이 격차를 알고 있고, 이미 반복적으로 써먹고 있습니다. 노출 여부와 관계없이 회전을 전제로 움직이는 것이 지금으로선 가장 안전한 기본값입니다.
