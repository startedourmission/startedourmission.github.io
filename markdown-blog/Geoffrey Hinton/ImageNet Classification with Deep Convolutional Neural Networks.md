...
Alex Krizhevsky는 우크라이나 출신 토론토대 박사과정 학생으로 GPU 프로그래밍에 뛰어났습니다. Ilya Sutskever는 이후 OpenAI 공동 창립자가 되었습니다. Geoffrey Hinton은 이 논문의 성공으로 2013년 Google에 합류했습니다.

## 요약

AlexNet은 5개의 합성곱 층과 3개의 완전 연결 층으로 구성된 신경망으로, 약 6천만 개의 파라미터와 65만 개의 뉴런을 포함합니다. 2개의 NVIDIA GTX 580 GPU를 활용한 병렬 훈련으로 6일 내에 학습을 완료했고, 총 200,000개의 이미지로 훈련된 후 검증 데이터에서 37.5%의 top-1 오류율과 15.3%의 top-5 오류율을 달성했습니다.
...
**아키텍처 설계:**
입력은 227x227 픽셀의 RGB 이미지이며, 11x11 필터(stride 4)부터 시작하여 점진적으로 작은 필터(3x3, stride 1)를 사용합니다. Max-pooling 층은 3x3 윈도우에서 stride 2로 공간 차원을 축소합니다.
...
ImageNet LSVRC-2010에서 top-1 오류율 37.5%, top-5 오류율 15.3%를 달성했으며, 이는 당시 최고 성능인 26.2%(top-5)보다 훨씬 우수했습니다. 신경망의 깊이가 매우 중요함을 보여주었으며, 5층 신경망은 3층 모델보다 2% 향상된 성능을 보였습니다.
