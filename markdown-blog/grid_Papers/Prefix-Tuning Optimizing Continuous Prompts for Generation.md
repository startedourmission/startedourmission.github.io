...
![[markdown-blog/grid_Papers/_assets/Prefix-Tuning_1.png]]
# 요약
...
"모델을 내 도메인에 맞게 쓰고 싶은데 전체 파인튜닝은 너무 비싸다"는 상황에서 출발하는 기술입니다.

**LoRA의 전신.** 지금 가장 많이 쓰이는 파인튜닝 기법인 LoRA는 Prefix-Tuning의 철학("모델 파라미터는 건드리지 않고 최소한의 학습 가능한 벡터만 추가한다")을 어텐션 가중치에 적용한 버전입니다. Prefix-Tuning을 이해하면 LoRA가 왜 그렇게 설계됐는지 바로 연결됩니다.

**도메인 특화 모델 구축.** 법률 문서 요약, 의료 기록 정리, 사내 용어 기반 응답 등 특정 스타일과 지식이 필요한 경우, 전체 파인튜닝 없이 PEFT 기법으로 베이스 모델을 특화할 수 있습니다. GPU 메모리와 비용이 일반 파인튜닝의 10~30분의 1 수준입니다.

**API 모델 활용 한계와 트레이드오프.** GPT-4, Claude API처럼 파라미터에 접근할 수 없는 모델은 Prefix-Tuning을 직접 적용할 수 없습니다. 이 경우 시스템 프롬프트 설계로 비슷한 효과를 내는 것이 현실적인 대안이며, Prefix-Tuning은 왜 시스템 프롬프트가 동작하는지를 이해하는 이론적 배경이 됩니다.