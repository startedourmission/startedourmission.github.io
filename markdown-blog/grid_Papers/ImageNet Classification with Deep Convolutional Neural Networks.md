...
aliases:
  - AlexNet
image: "![[alex_1.png]]"
description: 딥러닝 혁명을 일으킨 AlexNet 논문을 요약합니다. 8개 층으로 구성된 CNN 아키텍처와 함께, ReLU 활성화 함수, 드롭아웃, 데이터 증강 등 당시 혁신적이었던 기법들을 도입하여 ImageNet 대회에서 압도적인 성능을 달성한 과정을 설명합니다.
---
2012년, 딥러닝 대부 제프리 힌턴과 두 명의 대학원생은 컴퓨터 비전 분야에 혁명을 일으킬 논문을 발표합니다. 이 논문은 ImageNet 대회에서 기존 방법들을 압도적으로 뛰어넘는 성능을 보여주며 대 딥러닝 시대를 열었습니다. 논문에서 제안한 모델은 **AlexNet**이라고 불립니다. 단순히 우수한 성능을 보인 것이 아니라 현대 딥러닝의 핵심 기법들을 체계적으로 도입하여 실용적인 딥러닝의 가능성을 입증했습니다.
...
ImageNet은 **1500만 장 이상**의 고해상도 이미지로 구성된 대규모 데이터셋입니다. **ILSVRC는 이 중 약 1000개 클래스에서 각각 1000장**의 이미지를 선별하여 구성됩니다.
...
가장 중요한 혁신 중 하나는 기존의 tanh나 sigmoid 대신 **ReLU**를 사용한 것입니다.
$$ReLU = \max(0, x)$$
