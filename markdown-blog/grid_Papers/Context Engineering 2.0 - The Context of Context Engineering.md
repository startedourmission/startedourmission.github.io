...
![[블로그/markdown-blog/grid_Papers/_assets/1-ContextEngineering2.0.png]]
...
"프롬프트 엔지니어링"이라는 말이 "컨텍스트 엔지니어링"으로 진화하고 있는 이유가 이 논문에 있습니다.

**에이전트 시스템 설계의 프레임.** 단기 메모리(현재 대화)와 장기 메모리(사용자 프로파일, 과거 이력)를 분리해서 관리하는 구조는 Claude Code, LangGraph 같은 에이전트 도구가 모두 따르는 패턴입니다. 시스템을 만들 때 "이 정보는 어느 레이어에 넣어야 하나"를 고민하는 기준이 됩니다.

**정보 엔트로피 줄이기 = 좋은 프롬프트.** 논문의 핵심은 단순합니다. 모델이 의도를 추론해야 하는 불확실성을 줄일수록 출력 품질이 높아집니다. 실무에서 "프롬프트를 구체적으로 써라"는 조언이 왜 효과가 있는지 이론적 근거를 여기서 찾을 수 있습니다.

**컨텍스트 관리 전략 평가 기준.** 어떤 RAG·메모리 시스템이 더 좋은지 비교할 때 이 논문의 계층 분류를 기준으로 쓸 수 있습니다. "우리 시스템은 단기·장기 메모리를 어떻게 분리하고 있는가"를 물어보면 설계 수준이 바로 드러납니다.