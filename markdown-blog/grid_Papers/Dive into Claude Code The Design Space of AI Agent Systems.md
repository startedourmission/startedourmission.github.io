...
에이전트를 직접 만들거나 평가해야 하는 사람에게 가장 실용적인 논문 중 하나입니다.

**"에이전트 = 루프 + 하네스" 공식.** 논문이 보여주는 1.6%(핵심 루프) vs 98.4%(하네스) 비율은 충격적입니다. Claude Code가 강력한 이유는 모델이 아니라 권한 시스템·컨텍스트 압축·서브에이전트 조율 같은 하네스 설계에 있습니다. 자체 에이전트가 자꾸 무너진다면 루프가 아니라 하네스를 먼저 점검해야 합니다.

**5가치-13원칙 체크리스트로 활용.** 논문이 정리한 Claude Code의 설계 원칙 표를 자기 에이전트 시스템에 그대로 매핑해보는 것만으로도 설계 빈틈이 보입니다. "우리 시스템은 최소 권한 원칙을 코드 어디서 구현하고 있는가"처럼 구체적인 질문으로 연결됩니다.

**컨텍스트 압축 전략 참고.** 긴 작업일수록 컨텍스트 창이 터지는 문제가 생기는데, 이 논문은 Claude Code가 어떻게 압축·요약·서브에이전트 분리로 이 문제를 다루는지 코드 레벨까지 보여줍니다. 자체 에이전트에 바로 적용할 수 있는 패턴입니다.
