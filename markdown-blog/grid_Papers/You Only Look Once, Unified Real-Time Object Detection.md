...
**경계 상자 예측**: 각 셀은 BB개의 경계 상자(일반적으로 2개)를 예측하며, 각 상자는 다섯 가지 값: (x,y,w,h,confidence)(x,y,w,h,confidence)로 구성됩니다. 좌표 (x,y)(x,y)는 그리드 셀 경계에 대한 상대적인 상자 중심을 0과 1 사이로 정규화하여 나타냅니다. 치수 (w,h)(w,h)는 전체 이미지에 대한 상자의 너비와 높이를 나타냅니다. 신뢰도 점수는 상자에 객체가 포함될 확률과 경계 상자의 정확도를 모두 반영하며, 공식적으로는 Pr(Object)×IOUpredtruthPr(Object)×IOUpredtruth로 정의됩니다.
...
1. **사전 훈련**: 처음 20개의 컨볼루션 레이어는 ImageNet에서 $224 \times 224$ 해상도로 사전 훈련됩니다.
2. **감지 미세 조정**: 4개의 추가 컨볼루션 레이어와 2개의 완전 연결 레이어가 추가되고, 입력 해상도는 $448 \times 448$로 증가됩니다.
3. **손실 함수**: YOLO는 순진한 제곱 오차의 한계를 해결하기 위해 여러 수정 사항이 적용된 신중하게 설계된 제곱합 오차 손실을 사용합니다.

$$
\begin{aligned}
\mathcal{L}_{\text{loc}} = \;& \lambda_{\text{coord}} \sum_{i=0}^{S^2} \sum_{j=0}^{B} \mathbf{1}_{ij}^{\text{obj}}
...
손실 함수는 가중치 항 ($\lambda_{\text{coord}}=5$, $\lambda_{\text{noobj}}=0.5$)을 사용하여 위치 및 분류 오차의 균형을 맞추고, 상자 치수의 제곱근을 예측하여 다양한 객체 크기를 더 잘 처리합니다.
...
CCTV·드론·자율주행·제조 결함 검출 등 카메라가 붙은 곳 어디서나 YOLO 계열이 동작하고 있습니다.

**엣지 디바이스 배포.** YOLOv8, YOLOv11 같은 최신 버전은 NVIDIA Jetson, Raspberry Pi, 스마트폰에서 실시간으로 돌아갑니다. 클라우드 API 없이 카메라 옆 소형 컴퓨터 하나로 객체 탐지 서비스를 만들 수 있다는 의미입니다. 비용과 지연 시간 모두 압도적으로 유리합니다.

**커스텀 도메인 파인튜닝.** Ultralytics의 YOLOv8은 자체 라벨링 데이터 수백 장으로도 파인튜닝이 가능합니다. 공장 불량품 검출, 농장 해충 탐지, 매장 진열 상태 모니터링 등 도메인 특화 탐지기를 단 몇 시간 안에 만들 수 있습니다.

**멀티모달 AI와의 연결.** GPT-4V, Claude Vision 같은 멀티모달 모델이 "이미지를 이해"할 때, 내부적으로는 이미지를 패치로 분할해 처리하는 과정이 YOLO의 그리드 분할 철학과 이어집니다. 비전 AI의 흐름을 파악하려면 YOLO에서 시작하는 게 가장 직관적입니다.