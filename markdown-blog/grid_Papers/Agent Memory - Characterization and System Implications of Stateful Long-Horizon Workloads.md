...
공동 1저자는 [[야스민 오므리]]와 Ziyu Gan이고, 시니어 저자는 [[티에리 탐베]]입니다. [[티에리 탐베]]는 스탠퍼드에서 알고리즘부터 실리콘까지 통째로 설계하는 효율적 AI 하드웨어를 연구합니다. 모델을 어떻게 싸게 돌릴지를 회로 수준까지 내려가 보는 사람이 에이전트 메모리를 들여다봤다는 것 자체가, 이 연구가 "정확도"가 아니라 "비용"을 보려 했다는 신호입니다.

공저자 명단도 같은 방향을 가리킵니다. 임베디드 머신러닝과 저전력 하드웨어 가속기의 권위자 [[마리안 페르헐스트]](KU Leuven, imec)가 들어가 있어 GPU 에너지·대역폭 측정에 무게가 실렸고, 정보이론과 데이터 압축의 대가 [[차치 바이스만]]는 상호작용 기록을 압축해 기억으로 만드는 문제의 본질을 짚습니다. 계산 사회과학을 개척한 [[알렉스 펜틀런드]]는 에이전트가 사용자별로 장기 기억을 쌓아 사람을 대리하는 흐름에 오래 관심을 둬왔습니다. 정확도 벤치마크를 만드는 NLP 그룹이 아니라, 하드웨어와 시스템과 정보이론을 하는 사람들이 모였다는 점이 이 논문의 시각을 결정합니다.
