...
환각이 왜 생기는지 알면, 줄이는 전략도 달라집니다.

**"모른다"를 말하게 만들기.** 논문의 핵심 주장은 "모르겠다고 답하면 불이익을 받는 평가 구조가 환각을 유발한다"는 겁니다. 실무에서도 동일합니다. 프롬프트에 "확실하지 않으면 모른다고 말해줘"를 명시하는 것만으로 환각이 눈에 띄게 줄어드는 이유가 바로 이것입니다.

**RAG의 존재 이유.** 사전훈련 데이터에 없는 정보는 통계적으로 만들어낼 수밖에 없습니다. 검색 증강 생성(RAG)은 모델이 외부 문서를 직접 참조하게 해서 "지어내야 하는 상황" 자체를 줄입니다. 환각의 원인을 이해한 사람이 RAG를 더 잘 설계할 수 있습니다.

**AI 결과물 검수 기준 설정.** 환각이 완전히 제거되지 않는다는 사실을 인정하고, 고위험 영역(법률, 의료, 수치 데이터)과 저위험 영역을 분리해서 검수 비용을 배분하는 것이 현실적입니다. "AI를 믿을 수 있는가"가 아니라 "어떤 영역에서 어느 수준으로 믿을 수 있는가"로 질문을 바꾸는 것이 실무의 시작입니다.