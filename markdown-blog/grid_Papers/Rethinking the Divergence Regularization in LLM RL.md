...
[[Tencent Hunyuan]]과 싱가포르국립대(NUS)의 공동 연구다. Tencent 측에서는 [[Jiarui Yao]](UIUC 박사과정 + Hunyuan 인턴), [[Xiangxin Zhou]], [[Liefeng Bo]], [[Tianyu Pang]]이 참여했고, NUS에서는 [[Penghui Qi]]와 [[Wee Sun Lee]]가 합류했다.
...
- **SPO** (Simple Policy Optimization): PPO의 hard clipping을 smooth quadratic regularizer로 대체한 방법. Ratio 기반 trust region은 그대로지만 경계 밖에서도 corrective gradient를 제공한다.
- **DPPO**: Binary-TV 기반 trust region. Ratio 대신 절대 확률 이동량으로 경계를 정의한다.
...
아래 표가 네 방법의 trust region 설계를 한눈에 비교한다.
...
- **문제**: PPO/GRPO의 ratio 기반 trust region은 long-tail 어휘에서 저빈도 토큰을 과도하게 제약하고 고빈도 토큰을 과소 제약한다. DPPO가 Binary-TV로 이를 해소했지만 hard mask로 인해 경계 근처에서 gradient가 급변하고 corrective 신호가 없었다.
- **해법**: DPPO의 Binary-TV trust region에 SPO의 smooth quadratic regularizer를 결합해 DRPO를 제안. Gradient 가중치가 연속적이고 유계이며, trust region 밖에서는 자동으로 역방향 corrective 신호를 제공한다.
