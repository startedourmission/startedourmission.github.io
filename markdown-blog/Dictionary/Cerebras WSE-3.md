...
이 구조의 핵심 이점은 외부 메모리(HBM) 접근 없이 모델의 가중치를 온칩에 유지할 수 있다는 점이다. 덕분에 Llama 405B 같은 초대형 모델에서도 1,000 tok/s 이상의 추론 속도를 달성하며, 이는 GPU 기반 시스템 대비 월등한 수치이다. 125 PFLOPS의 연산 성능과 결합하여, 초저지연 대규모 추론에 특화된 아키텍처로 평가받고 있다.
