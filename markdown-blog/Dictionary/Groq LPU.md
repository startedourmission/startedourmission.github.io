...
[[Cerebras WSE-3]]와 함께 비GPU 추론 하드웨어의 대표 주자이다. LPU는 GPU나 TPU와 근본적으로 다른 설계 철학을 가진다. HBM 대신 온칩 SRAM만을 사용하여 메모리 접근 지연을 최소화했으며, 이를 통해 예측 가능하고 일관된 성능을 제공한다. TTFT(첫 번째 토큰 생성까지의 시간)가 100ms 미만으로, 체감 응답 속도가 극히 빠르다.
