...
칸반(Kanban)은 원래 도요타 생산방식(Toyota Production System)에서 개발된 재고 관리 기법으로, 일본어로 "간판(看板)"을 뜻합니다. 소프트웨어 개발로의 적용은 [[David J. Anderson]]이 2010년 저서 *Kanban: Successful Evolutionary Change for Your Technology Business*로 체계화했습니다.

[[Scrum]]이나 [[Extreme Programming]]과 달리 **이터레이션이 없고**, **역할이 없으며**, **미팅 규정이 없습니다**. 현재 프로세스를 유지하면서 점진적으로 개선하는 것을 원칙으로 합니다.
...
Scrum의 이벤트 구조에 Kanban의 WIP 제한을 결합한 하이브리드로, 실무에서 가장 흔한 방식 중 하나입니다.
