...
description: "마르코프 결정 과정의 가치 함수를 정의하고, 최적 정책을 구하는 가치 반복과 정책 반복 알고리즘, 상태 전이 확률 추정 방법을 다룹니다"
---
...
[[가치 반복]](value iteration)은 $V^*$를 계산하는 반복 알고리즘이다.
...
[[정책 반복]](policy iteration)은 MDP를 풀기 위한 또 다른 고전적 알고리즘이다. 가치 반복이 $V^*$에 초점을 맞추는 반면, 정책 반복은 **정책 $\pi$ 자체**에 초점을 맞춘다.
