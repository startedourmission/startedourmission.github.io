...
넘파이는 효율적인 대규모 배열 연산을 위해 사용하는 파이썬 라이브러리입니다. 내부에서 C언어 기반으로 작동되기때문에 계산량이 많을 때 속도가 느려지는 Python의 단점을 줄여줍니다.
또한 PyTorch의 텐서 자료형과도 호환이 되기때문에, 데이터 분석, 과학 계산, 머신러닝 등 다양한 분야에서 필수적으로 사용됩니다.
