...
tags:
  - 정보
description: "모델 가중치를 건드리지 않고 시스템 프롬프트, 도구, 실행 흐름만 바꿔서 성능을 끌어올리는 하네스 엔지니어링. LangChain은 하네스만 바꿔서 Terminal Bench 30위권 밖에서 5위권으로 올라갔습니다."
---
## 결론부터 말하면
...
## 프롬프트 → 컨텍스트 → 하네스 → 루프: 진화의 흐름

네 개념의 관계를 이해하면 에이전트 시스템 설계의 전체 구조가 보입니다.
...
[[Andrej Karpathy]]가 주목을 끌고, LangChain과 Anthropic이 공식적으로 정의하면서 부상했습니다. [[RAG]], MCP, 메모리 등을 활용한 시스템 수준의 컨텍스트 설계입니다. "LLM에 주입되는 모든 토큰의 설계와 관리"가 범위입니다.

### 하네스 엔지니어링 (2026년 2월~)

컨텍스트 엔지니어링의 범위를 넘어, LLM "바깥"에서 설계해야 하는 모든 것을 포함합니다. 린터, CI 파이프라인, 병렬 에이전트 간 충돌 방지, 아키텍처 제약의 기계적 강제, 엔트로피 관리 등입니다.

중요한 건, 이 네 가지가 대체 관계가 아니라 **포함 관계**라는 점입니다. 루프 엔지니어링 안에 하네스 엔지니어링이 있고, 하네스 엔지니어링 안에 컨텍스트 엔지니어링이 있고, 컨텍스트 엔지니어링 안에 프롬프트 엔지니어링이 있습니다. 각각의 영역이 계속 유효하면서, 더 넓은 범위로 확장된 것입니다.

### 루프 엔지니어링 (2026년 6월~)

하네스라는 공간이 안정화되는 시점에, 에이전트가 그 공간 안에서 어떻게 반복 개선하는지를 설계하는 개념이 등장했습니다. 루프 엔지니어링(Loop Engineering)입니다.

[[Claude Code]]를 이끄는 Boris Cherny가 이렇게 말했습니다. "나는 더 이상 Claude에게 직접 프롬프트하지 않습니다. Claude에게 프롬프트하는 루프를 직접 실행하고 있습니다. 내 일은 루프를 짜는 것입니다." OpenClaw를 만든 Peter Steinberger(현 OpenAI)도 같은 시점에 같은 메시지를 냈습니다. "에이전트에게 프롬프트하는 걸 그만하세요. 에이전트에게 프롬프트하는 루프를 설계하세요."

하네스 엔지니어링이 "에이전트가 작업할 환경"을 만드는 것이라면, 루프 엔지니어링은 "에이전트가 그 환경 안에서 어떻게 목표를 향해 반복하는지"를 설계하는 것입니다. 공간과 엔진의 관계입니다.
...
## 루프 엔지니어링: 하네스 안에서 돌아가는 것

### 루프의 5단계

모든 에이전트 루프는 같은 사이클을 반복합니다.

**DISCOVER → PLAN → EXECUTE → VERIFY → ITERATE**

검증을 통과하면 완료됩니다. 통과하지 못하면 다시 돌아갑니다. 에이전트가 하나든 수십 개의 플릿이든 이 구조는 동일합니다.

하네스의 구성요소가 이 사이클을 어떻게 지지하는지를 보면 둘의 관계가 명확해집니다.

- **Automations**는 DISCOVER를 트리거합니다. 루프가 언제 시작할지를 정합니다.
- **Worktrees**는 EXECUTE를 병렬화합니다. 여러 에이전트가 같은 저장소에서 충돌 없이 작업합니다.
- **Skills**는 DISCOVER를 빠르게 만듭니다. 에이전트가 프로젝트를 매 루프마다 처음부터 파악하지 않아도 됩니다.
- **Subagents**는 VERIFY를 독립적으로 만듭니다. 만든 에이전트와 검증하는 에이전트를 분리합니다.
- **Memory**는 DISCOVER를 누적시킵니다. 루프 47회차가 1에서 46회차에서 이미 시도한 것을 압니다.

### 단일 에이전트 루프 vs 플릿 루프

스케일에 따라 두 가지 형태로 나뉩니다.

**단일 에이전트 루프**는 하나의 에이전트가 사이클 전체를 담당합니다. 탐색하고, 계획하고, 실행하고, 검증하고, 수정하는 것까지 혼자 합니다. 범위가 좁고 목표가 명확한 작업에 적합합니다. Anthropic의 이중 에이전트 하네스에서 코딩 에이전트가 세션마다 기능 하나를 점진적으로 구현하고 커밋하는 패턴이 이 방식입니다.

**플릿 루프**는 오케스트레이터가 목표를 분해하고, 각 조각을 스페셜리스트 에이전트에게 넘깁니다. 스페셜리스트들도 자신의 서브에이전트를 가질 수 있습니다. 트리 전체의 모든 에이전트가 각자의 루프를 돌면서, 오케스트레이터가 최종 완료를 판단합니다. 코드베이스 전체 리팩토링이나 며칠 단위의 장기 작업에 씁니다.

### 오픈 루프 vs 클로즈드 루프

실무에서 가장 중요한 구분입니다.

**오픈 루프**는 에이전트에게 목표만 주고 탐색 공간을 열어둡니다. 어떤 경로로 갈지, 얼마나 시도할지를 에이전트가 스스로 결정합니다. 기대보다 더 나은 결과를 가져오기도 합니다. 단점은 비용입니다. 중간 규모 코딩 작업 하나에 5만에서 20만 토큰, 오케스트레이터와 스페셜리스트 3명 구성의 플릿은 50만에서 200만 토큰이 필요합니다. 스케줄에 따라 매일 돌리면 주당 수백만 토큰입니다. API 예산이 넉넉하지 않은 팀에게 오픈 루프는 아직 현실적이지 않습니다.

**클로즈드 루프**는 사람이 경로를 먼저 설계합니다. 목표, 단계, 각 단계의 평가 기준, 루프가 끝날 조건을 명시합니다. 에이전트는 그 프레임 안에서 루프를 돕니다. 매 루프마다 품질 게이트가 있어서 결과가 누적되어 개선됩니다. 비용이 오픈 루프보다 적고 예측 가능합니다. 품질 게이트 없이 작동하는 에이전트는 드리프트합니다. 게이트가 있어야 수렴합니다.

오늘 실제로 작동하는 루프는 대부분 클로즈드입니다. 오픈 루프는 하네스와 클로즈드 루프가 충분히 검증된 다음 단계의 이야기입니다.

### 프롬프트 엔지니어 vs 루프 엔지니어

프롬프트 엔지니어는 더 나은 지시를 씁니다. 언어 능력의 문제입니다. 더 좋은 프롬프트가 더 나은 단일 출력을 만들고, 출력 이후에는 사람이 검토하고 다음 프롬프트를 씁니다. 사람이 피드백 루프의 일부입니다.

루프 엔지니어는 더 나은 피드백 사이클을 설계합니다. 소프트웨어 엔지니어링의 문제입니다. 더 좋은 루프가 검증된 결과를 만들고, 시스템이 스스로 검토하고 수정합니다. 시스템이 피드백 루프입니다.

차이를 한 줄로 쓰면: 프롬프트 엔지니어는 에이전트에게 출력을 요청하고, 루프 엔지니어는 에이전트가 검증된 결과를 생성하는 시스템을 만듭니다. 하네스 엔지니어링이 그 시스템의 공간을 만드는 일이라면, 루프 엔지니어링은 그 공간에 엔진을 넣는 일입니다.
...
- [11] S. Rahul, "Loops: What Every AI Engineer Needs to Know in 2026," x.com/sairahul1, Jun. 9, 2026.