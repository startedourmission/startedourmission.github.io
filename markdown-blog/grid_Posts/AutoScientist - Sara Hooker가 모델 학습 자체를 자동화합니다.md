...
tags:
  ...
  - 머신러닝
  - AI평가
description: "전 Cohere VP of Research [[Sara Hooker]]가 공동창업한 Adaption Labs가 첫 제품 AutoScientist를 공개했습니다. 모델 학습·정렬 전체 research loop을 자동화하는 시스템으로, 데이터 큐레이션과 학습 recipe를 동시에 self-improve 합니다. 사내 AI 리서처가 직접 설정한 학습 대비 평균 +35%, win rate는 48%에서 64%로 올랐다고 합니다. 첫 30일 무료, $50M 시드 (Emergence Capital + Mozilla Ventures 리드)."
---
...
이걸 만든 사람이 누구냐가 중요합니다. CEO [[Sara Hooker]]는 *Cohere For AI* 를 이끌었던 전 Cohere VP of Research이고, 작은 모델로 큰 효율을 내는 *"scaling race에 반대"* 노선의 학계 대표자 중 한 명입니다. 공동창업자 Sudip Roy는 전 Cohere inference director입니다. *프론티어 랩의 학습 노하우를 외부 도구로 옮기는* 작업을 두 사람이 회사로 만든 것이죠.
